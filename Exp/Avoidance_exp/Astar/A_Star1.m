...
function waypoint_pos = A_Star1(factor_shrink)
MAX_X=20;
MAX_Y=20;
MAX_VAL=10;
...
grid on;
% set(gca,'XTick',[],'YTick',[])
xlabel('pos_x=x*factor_shrink-MAX_X*factor_shrink/2');
ylabel('pos_y=y*factor_shrink-MAX_Y*factor_shrink/2');
hold on;
...
% disp('length:')
length = size(Optimal_path,1);
reverse_pos=Optimal_path*factor_shrink-MAX_X*factor_shrink/2;
for i=1:length
    waypoint_pos(i,:)=reverse_pos(length+1-i,:); 
end
end
